package com.engineerpro.rest.example.unit.controller;
...
public class TransactionControllerTest {
  ...
  public void testProperty() {
    assertThat(myProperty).isEqualTo("jdbc:h2:mem:testdb");
  }

  @Test
  public void testGetTransactionHistory() throws Exception {
    // given
    when(transactionService.getUserTransactions(any()))
        .thenReturn(new GetUserTransactionsResponse(Arrays.asList(Transaction.builder().id(123456).idempotentKey("abc")
            .amount(1).balanceAfterTransaction(99).balanceBeforeTransaction(100).userId(1).build())));
    // when - then
  ...
  public void testCreateTransaction() throws Exception {
    ...
    String payload = objectMapper.writeValueAsString(
        CreateTransactionRequest.builder().amount(1).idempotentKey("an-idempotent-key").userId(1).build());
